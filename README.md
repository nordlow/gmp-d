...
D-language high-level wrapper for [GNU MP (GMP) library](GNU MP (GMP) library
that) that aims to be compatible with `std.bigint.BigInt` and be`@safe pure
nothrow @nogc`when possible.
